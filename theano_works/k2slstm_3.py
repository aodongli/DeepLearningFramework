...
s_word_emb = theano.shared(np.array(0, dtype='float32'))
options = {}
...
options['fluctuate_times'] = 10
options['save_to'] = 'lstm_3.npz'
# word vector size = 50
...
def lstm_decoder(y_prev, h_list, params, k_1, k_2):
    
    def _lstm_decoder(h): # Pay attention to the parameter order
        def _slice(_x, n, dim):
            # For n: 0-i, 1-f, 2-o, 3-c
            return _x[n * dim:(n+1) * dim]
        ...
        return y_h_c[-2], y_h_c[-1] # y_temp at each step
...
def build_model_for_training(params):
    ...
    _, y_list = lstm_decoder(y_prev, h, params, k_1, k_2) # T.shape() returns array([len(row), len(column)])
    ...
    return X, y_prev, Y, k_1, k_2, cost # output every predicted word

def sgd(lr, tparams, grads, X, y_prev, k_1, k_2, Y, cost):
    ...
    # updates the weights.
    f_grad_shared = theano.function([X, y_prev, k_1, k_2, Y],
                                     cost, 
                                     updates=gsup,
...
def adadelta(lr, tparams, grads, X, y_prev, k_1, k_2, Y, cost):
    """
    ...
    f_grad_shared = theano.function([X, y_prev, k_1, k_2, Y], cost, updates=zgup + rg2up,
...
def training_process(raw_data_file = 'combinedlyricsv3'):
    ...
    word_misc = getVoc('word_vec_sub.md', 'stopwords_zh.txt', False)
    ...
    ######### GLOBAL
    global s_word_emb
    s_word_emb = theano.shared(word_emb)
    #########
    ######
    ...
    X, y_prev, Y, k_1, k_2, cost = build_model_for_training(params)
    ...
    f_grad_shared, f_update = sgd(lrate, params, grads, X, y_prev, k_1, k_2, Y, cost)
    print "[Complete]\n"
    ...
    for epoch in range(options['max_epochs']):
        ...
        for iteration in range(options['batch_size']):
            ...
            #print np.shape(sample_in_vector), np.shape(EOS), np.shape(word_emb), np.shape(sample_out_1hot)
            cost_sample = f_grad_shared(sample_in_vector, EOS, num_ps, max_len_4_out, sample_out_1hot) # y_i is decoder's temporal outputs
            ...
            if np.isnan(cost_sample) or np.isinf(cost_sample):
                print "Bad cost detected on", epoch, "th epoch:", cost_sample
                return 1., 1., 1.
        ...
        if difference < 0.00001 and difference > 0:
...
def build_model_for_prediction(params):
    ...
    h = lstm_encoder(X, params)
    y_list, _ = lstm_decoder(y_prev, h, params, k_1, k_2) # T.shape() returns array([len(row), len(column)])
    #theano.function([y_prev, h, word_emb, k], y_h_c)
    #print y_h_c[-1].type, y_h_c[-1][0].type
    f_predict = theano.function([X, y_prev, k_1, k_2], y_list, name='predict_f', mode = 'FAST_RUN', allow_input_downcast=True)
...
def prediction_process(test):
    ...
    print "Building model..."
    f_predict = build_model_for_prediction(params)
    ...
    y_i = f_predict(sample_vector, EOS, len(test), 15)
    for i in y_i:
        print(id2word[i])
