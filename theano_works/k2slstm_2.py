...
def lstm_decoder(y_prev, h_list, params, word_emb, k_1, k_2):
...
def build_model_for_training(params):
    ...
    y_list = lstm_decoder(y_prev, h, params, word_emb, k_1, k_2) # T.shape() returns array([len(row), len(column)])
    ...
    return X, y_prev, word_emb, Y, k_1, k_2, cost # output every predicted word

def sgd(lr, tparams, grads, X, y_prev, word_emb, k_1, k_2, Y, cost):
    ...
    # updates the weights.
    f_grad_shared = theano.function([X, y_prev, word_emb, k_1, k_2, Y],
                                     cost, 
                                     updates=gsup,
...
def adadelta(lr, tparams, grads, X, y_prev, word_emb, k_1, k_2, Y, cost):
    """
    ...
    f_grad_shared = theano.function([X, y_prev, word_emb, k_1, k_2, Y], cost, updates=zgup + rg2up,
...
def training_process(raw_data_file = 'combinedlyricsv3'):
    ...
    X, y_prev, _word_emb, Y, k_1, k_2, cost = build_model_for_training(params)
    ...
    f_grad_shared, f_update = sgd(lrate, params, grads, X, y_prev, _word_emb, k_1, k_2, Y, cost)
    print "[Complete]\n"
    ...
    for epoch in range(options['max_epochs']):
        ...
        for iteration in range(options['batch_size']):
            ...
            cost_sample = f_grad_shared(sample_in_vector, EOS, word_emb, num_ps, max_len_4_out, sample_out_1hot) # y_i is decoder's temporal outputs
